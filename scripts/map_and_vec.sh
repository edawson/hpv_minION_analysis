...
do
    ...
    ${vg} map -t 4 -GX -B ${bandwidth} -r tmpError.txt -x hpv_16.xg -g hpv_16.gcsa | ${vg} vectorize -x hpv_16.xg - > hpv_16_Aligned.error${error_rate}.bandwidth${bandwidth}vecs.txt
done


for indel_rate in 0.0000 0.01 0.05 0.15 0.25
do
     ${vg} map -t 4 -GX -B ${bandwidth} -r hpv_*error0.0.*indel${indel_rate}.txt -x hpv_16.xg -g hpv_16.gcsa | ${vg} vectorize -x hpv_16.xg - > hpv_16_Aligned.indel${indel_rate}.vecs.txt
done
